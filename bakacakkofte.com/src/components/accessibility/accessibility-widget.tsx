  ...
  return (
    <>
      <button
        ...
        className="fixed left-4 bottom-4 z-40 inline-flex h-12 w-12 items-center justify-center rounded-full border-2 border-red bg-bone text-red shadow-lg transition-transform hover:-translate-y-0.5 md:left-6 md:bottom-6 md:h-14 md:w-14"
      >
        <svg
          viewBox="0 0 24 24"
        ...
        {hasChanges && (
          <span
            aria-hidden="true"
            className="absolute -right-0.5 -top-0.5 h-3 w-3 rounded-full bg-gold"
          />
        )}
      </button>
      ...
      {open && (
        <div
          className={`fixed inset-0 z-[100] flex items-center justify-center bg-ink/50 p-4 backdrop-blur-sm transition-opacity duration-300 ${
        ...
        >
          <div
            ...
            className={`relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-peach bg-bone text-ink shadow-2xl transition-all duration-300 ${
              mounted ? "scale-100 opacity-100" : "scale-95 opacity-0"
            }`}
          >
            <div className="flex items-start justify-between gap-4 border-b border-peach bg-cream px-6 py-5">
              <div>
                ...
                <h2
                  id="a11y-title"
                  className="display-font mt-1 text-[24px] font-bold leading-tight md:text-[28px]"
                >
              ...
              <button
                ...
                aria-label="Kapat"
                className="shrink-0 rounded-lg border border-peach p-2 transition-colors hover:border-red hover:text-red"
              >
                <svg
                  viewBox="0 0 24 24"
                  className="h-4 w-4"
            ...
            <div className="flex items-center justify-between border-t border-peach px-6 py-4">
              <button
                type="button"
                onClick={resetAll}
                disabled={!hasChanges}
              ...
              <button
                type="button"
                onClick={() => setOpen(false)}
                className="rounded-full bg-red px-5 py-2 text-[13px] font-semibold text-bone transition-colors hover:bg-red-deep"
              >
                Tamam
              </button>
            </div>
