...
function CategoryToggle({ label, description, checked, disabled, onChange }: CategoryToggleProps) {
  return (
    <div className="flex items-start justify-between gap-4 py-4 border-b border-peach last:border-0">
      <div className="flex-1">
        <p className="display-font font-semibold text-ink text-sm">{label}</p>
        <p className="text-ink-2 text-sm mt-1 opacity-70">{description}</p>
      </div>
      <button
        role="switch"
        ...
        className={`relative w-11 h-6 rounded-full transition-colors duration-300 shrink-0 ${
          checked ? "bg-red" : "bg-ink/15"
        } ${disabled ? "opacity-60 cursor-not-allowed" : "cursor-pointer"}`}
      >
        <span
          className={`absolute top-0.5 left-0.5 w-5 h-5 bg-bone rounded-full shadow transition-transform duration-300 ${
  ...
  return (
    <>
      {!showPreferences && (
        ...
        >
          <div className="max-w-4xl mx-auto bg-bone rounded-2xl shadow-2xl border border-peach p-6 sm:p-8">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h3 className="display-font font-bold text-ink text-lg mb-2">Çerez Kullanımı</h3>
                <p className="text-ink-2 text-base leading-relaxed mb-5 opacity-80">
                  Deneyiminizi iyileştirmek için çerez kullanıyoruz. Detaylar için{" "}
                  <a href="/cerez-politikasi" className="text-red underline hover:opacity-80">Çerez Politikamızı</a>{" "}
                  inceleyebilirsiniz.
                </p>
                <div className="flex flex-col sm:flex-row gap-3">
                  <button onClick={handleAcceptAll} className="btn-red text-sm">
                    <Check className="w-4 h-4" /> Tümünü Kabul Et
                  </button>
                  <button onClick={handleRejectAll} className="btn-outline text-sm">
                    <X className="w-4 h-4" /> Tümünü Reddet
                  </button>
                  <button onClick={() => setShowPreferences(true)} className="btn-outline text-sm">
      ...
      {showPreferences && (
        <div
          className={`fixed inset-0 z-[95] bg-ink/60 flex items-center justify-center p-4 transition-opacity duration-300 ${
            mounted ? "opacity-100" : "opacity-0"
          }`}
          onClick={() => { setShowPreferences(false); if (getConsentState()) close(); }}
        >
          <div
            className={`bg-bone rounded-2xl shadow-2xl border border-peach max-w-lg w-full max-h-[85vh] overflow-y-auto transition-all duration-300 ${
          ...
          >
            <div className="p-6 sm:p-8">
              <div className="flex items-center justify-between mb-6">
                <h3 className="display-font font-bold text-ink text-lg">Çerez Tercihleri</h3>
                <button onClick={() => { setShowPreferences(false); if (getConsentState()) close(); }} className="p-1.5 rounded-lg hover:bg-ink/5 transition-colors" aria-label="Kapat">
              ...
              <div className="flex flex-col sm:flex-row gap-3 mt-6">
                <button onClick={handleSavePreferences} className="btn-red flex-1 justify-center text-sm">Seçimleri Kaydet</button>
                <button onClick={handleAcceptAll} className="btn-outline flex-1 justify-center text-sm">Tümünü Kabul Et</button>
              </div>
            </div>
          </div>
        </div>
