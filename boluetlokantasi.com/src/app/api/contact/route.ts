...
export async function POST(request: NextRequest) {
  ...
  // Honeypot check (if _honeypot field is filled, it's a bot)
  if ((body as Record<string, unknown>)._honeypot) {
    // Silently accept but don't process (bot trap)
    return NextResponse.json({ success: true });
  }

  // Send via FormSubmit (email stays server-side)
  const CONTACT_EMAIL = process.env.CONTACT_EMAIL || "info@boluetlokantasi.com";
  ...
  try {
    const response = await fetch(`https://formsubmit.co/ajax/${CONTACT_EMAIL}`, {
      ...
      body: JSON.stringify({
        ...
        _subject: `Bolu Et Lokantası - ${name.slice(0, 50)}`,
      }),
    });
