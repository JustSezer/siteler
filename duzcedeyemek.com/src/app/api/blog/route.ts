...
export async function POST(request: NextRequest) {
  ...
  if (title.length > 200 || excerpt.length > 1000 || content.length > 100_000) {
    return NextResponse.json({ error: "Field length exceeded" }, { status: 400, headers: corsHeaders() });
  }
  ...
  if (cover_image && typeof cover_image === "string") {
    if (!isValidImageUrl(cover_image)) {
      return NextResponse.json({ error: "Image URL not from trusted domain" }, { status: 400, headers: corsHeaders() });
    }
  }

  const existing = await getPostBySlug(safeSlug);
  if (existing) {
    return NextResponse.json({ error: "Slug already in use" }, { status: 409, headers: corsHeaders() });
  }
