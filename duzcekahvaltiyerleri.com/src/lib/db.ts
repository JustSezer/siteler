...
export function getPostBySlug(slug: string): Post | undefined {
  return getStmt("postBySlug", `
    ...
    FROM posts p
    LEFT JOIN categories c ON p.category_id = c.id
    WHERE p.slug = ? AND p.is_published = 1 AND p.published_at <= datetime('now')
...
export function updatePost(id: number, data: Record<string, unknown>): Database.RunResult {
  ...
  for (const [key, value] of Object.entries(data)) {
    ...
    switch (key) {
      ...
      case "slug":
        fields.push("slug = ?");
        values.push(String(value).substring(0, 200).toLowerCase().replace(/[^a-z0-9-]/g, ""));
        break;
      case "excerpt":
        fields.push("excerpt = ?");
        values.push(String(value).substring(0, 500));
        break;
      case "content":
        fields.push("content = ?");
        values.push(String(value).substring(0, 100000));
        break;
      case "cover_image":
        fields.push("cover_image = ?");
        values.push(value ? String(value).substring(0, 500) : null);
        break;
      case "category_id":
        fields.push("category_id = ?");
        values.push(value ? Math.floor(Number(value)) : null);
        break;
      case "tags":
        fields.push("tags = ?");
        values.push(value ? String(value).substring(0, 500) : null);
      ...
      case "reading_time":
        ...
        values.push(Math.max(1, Math.min(Math.floor(Number(value) || 5), 120)));
        break;
      case "is_published":
      ...
      case "published_at":
        ...
        values.push(value ? String(value).substring(0, 30) : new Date().toISOString().replace("T", " ").substring(0, 19));
        break;
    }
  }
...
export function createCategory(name: string, slug: string): Database.RunResult {
  ...
  if (!safeName || !safeSlug) {
    throw new Error("Geçersiz kategori adı veya slug.");
  }

  return getStmt("createCat", "INSERT INTO categories (name, slug) VALUES (?, ?)").run(safeName, safeSlug);
}

export function updateCategory(id: number, name: string, slug: string): Database.RunResult {
  const safeId = Math.floor(Number(id));
  if (!Number.isFinite(safeId) || safeId <= 0) {
    throw new Error("Geçersiz ID.");
  }
  ...
  return getDb().prepare("UPDATE categories SET name = ?, slug = ? WHERE id = ?").run(safeName, safeSlug, safeId);
}
...
export function getSubscriberByEmail(email: string): Subscriber | undefined {
  ensureSubscribersTable();
  const safe = String(email).substring(0, 254).toLowerCase().trim();
  return getDb().prepare("SELECT * FROM subscribers WHERE email = ?").get(safe) as Subscriber | undefined;
}

export function createSubscriber(email: string): Database.RunResult {
  ensureSubscribersTable();
  const safe = String(email).substring(0, 254).toLowerCase().trim();
  return getDb().prepare("INSERT INTO subscribers (email) VALUES (?)").run(safe);
}

export function deleteSubscriber(id: number): Database.RunResult {
  ensureSubscribersTable();
  const safeId = Math.floor(Number(id));
  if (!Number.isFinite(safeId) || safeId <= 0) throw new Error("Geçersiz ID.");
  return getDb().prepare("DELETE FROM subscribers WHERE id = ?").run(safeId);
}

export function unsubscribeByEmail(email: string): Database.RunResult {
  ensureSubscribersTable();
  const safe = String(email).substring(0, 254).toLowerCase().trim();
  return getDb().prepare("DELETE FROM subscribers WHERE email = ?").run(safe);
}
