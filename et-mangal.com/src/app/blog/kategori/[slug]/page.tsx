...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${cat.name} Yazıları | Et & Mangal Blog`,
    description: `${cat.name} kategorisindeki tüm yazılar. Et tarifleri, mangal teknikleri ve ${cat.name.toLowerCase()} hakkında rehberler.`,
        ...
        {categories.map((c) => (
          <Link
            ...
            className={`px-3 py-1.5 rounded-full text-xs sm:text-sm font-medium transition-colors ${
              c.slug === slug ? "bg-red-700 text-white" : "bg-stone-200 text-stone-700 hover:bg-stone-300"
            }`}
          >
            {c.name} ({c.count})
          </Link>
