...
function CategoryToggle({ label, description, checked, disabled, onChange }: CategoryToggleProps) {
  return (
    <div className="flex items-start justify-between gap-4 py-4 border-b border-rule last:border-0">
      <div className="flex-1">
        <p className="font-display font-semibold text-ink text-sm">{label}</p>
        <p className="text-ink-soft text-sm mt-1">{description}</p>
      </div>
      <button
        role="switch"
  ...
  return (
    <>
      {!showPreferences && (
        ...
        >
          <div className="max-w-4xl mx-auto bg-paper rounded-2xl shadow-2xl border border-rule p-6 sm:p-8">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  <button onClick={() => setShowPreferences(true)} className="inline-flex items-center justify-center gap-2 px-6 py-2.5 border border-rule text-ink font-semibold text-sm rounded-full hover:bg-ink/5 transition-colors">
      ...
      {showPreferences && (
        ...
        >
          <div
            className={`bg-paper rounded-2xl shadow-2xl border border-rule max-w-lg w-full max-h-[85vh] overflow-y-auto transition-all duration-300 ${
          ...
          >
            <div className="p-6 sm:p-8">
              ...
              <div className="flex flex-col sm:flex-row gap-3 mt-6">
                ...
                <button onClick={handleAcceptAll} className="flex-1 inline-flex items-center justify-center gap-2 px-6 py-2.5 border border-rule text-ink font-semibold text-sm rounded-full hover:bg-ink/5 transition-colors">Tümünü Kabul Et</button>
              </div>
            </div>
          </div>
        </div>
