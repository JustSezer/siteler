...
export default function PostEditor({ post }: { post?: Post }) {
  ...
  const [category, setCategory] = useState(post?.category || "Wix Başlangıç");
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 sm:p-6 space-y-4">
        <div>
          ...
          <input
            ...
            placeholder="Wix ile ilgili yazı başlığı"
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            required
            className="w-full px-3 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm font-mono"
            placeholder="wix-site-kurma-rehberi"
          />
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 sm:p-6">
        <label htmlFor="excerpt" className="block text-sm font-medium text-slate-700 mb-1">
          ...
          <span className="text-slate-400 font-normal ml-1">(SEO description olarak kullanılır)</span>
        </label>
        <textarea
          id="excerpt"
          value={excerpt}
          ...
          placeholder="Wix'te ... konusunu anlatan kısa açıklama (150-160 karakter önerilir)"
        />
        <p className="text-xs text-slate-400 mt-1">{excerpt.length}/1000</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 sm:p-6">
        ...
        <textarea
          ...
          required
          rows={18}
          className="w-full px-3 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm font-mono resize-y"
          placeholder="<h2>Wix ile...</h2><p>Bu yazıda...</p>"
        />
        <p className="text-xs text-slate-400 mt-1">
          HTML: h2, h3, p, ul, ol, li, strong, em, a, blockquote, code, table
        </p>
      </div>

      {/* Meta */}
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 sm:p-6 space-y-4">
        <h3 className="font-semibold text-slate-800">Yazı Detayları</h3>
        ...
        <div>
          ...
          <input
            ...
            maxLength={500}
            className="w-full px-3 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm"
            placeholder="wix, site kurma, tasarım (virgülle ayırın)"
          />
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-sm font-medium text-slate-700">Taslak</span>
            </label>
          </div>
          ...
          <div className="flex gap-3 w-full sm:w-auto">
            <Link
              href="/admin"
              className="flex-1 sm:flex-initial text-center border border-slate-300 text-slate-700 hover:bg-slate-50 font-medium px-5 py-2.5 rounded-lg text-sm transition-colors"
            >
              İptal
            </Link>
            <button
              type="submit"
              disabled={saving}
              className="flex-1 sm:flex-initial bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-medium px-5 py-2.5 rounded-lg text-sm transition-colors"
            >
