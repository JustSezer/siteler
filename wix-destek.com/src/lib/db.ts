...
export async function getAllPosts(limit?: number): Promise<Post[]> {
  await initDb();
  let query = "SELECT * FROM posts WHERE is_published = 1 ORDER BY published_at DESC";
  if (limit) {
...
export async function createLead(lead: Omit<Lead, "id" | "created_at" | "is_read">): Promise<void> {
  await initDb();
  await db.execute({
    sql: "INSERT INTO leads (name, email, phone, subject, message) VALUES (?, ?, ?, ?, ?)",
    args: [lead.name, lead.email, lead.phone || null, lead.subject, lead.message],
  });
}
...
export async function adminMarkLeadRead(id: number): Promise<void> {
  await initDb();
  await db.execute({ sql: "UPDATE leads SET is_read = 1 WHERE id = ?", args: [id] });
}

export async function getUnreadLeadCount(): Promise<number> {
  await initDb();
  const result = await db.execute("SELECT COUNT(*) as c FROM leads WHERE is_read = 0");
...
export async function adminGetStats(): Promise<{
  ...
  const totalLeads = await db.execute("SELECT COUNT(*) as c FROM leads");
  const unreadLeads = await db.execute("SELECT COUNT(*) as c FROM leads WHERE is_read = 0");
  return {
    ...
    totalLeads: totalLeads.rows[0].c as number,
    unreadLeads: unreadLeads.rows[0].c as number,
  };
}
